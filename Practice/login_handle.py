...
class LoginHandle:
    def __init__(self):
        self.login_page = LoginPage()      #LoginPage：获取登录页面所有的页面元素信息，用户名、密码
    #操作登录页面元素

    #去登陆
    def click_my(self):
        self.login_page.get_my().click()
    def click_my_login(self):
        self.login_page.get_my_login().click()
    def click_getlogin(self):
        self.login_page.get_login().click()
    def click_pwd(self):
        self.login_page.get_password_login().click()
